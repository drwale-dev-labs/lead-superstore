  ...
  if (query.isError || !query.data) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-16 text-center">
        ...
        <Link href="/shop" className="mt-6 inline-block text-sm text-amber-700 hover:underline">
          Back to shop
        </Link>
      </div>
    );
  ...
  if (result.status === "success") {
    return (
      <div className="mx-auto max-w-2xl px-6 py-16 text-center">
        ...
        <h1 className="mt-4 text-xl font-semibold text-stone-900">Payment successful</h1>
        <p className="mt-2 text-sm text-stone-600">
          Order <strong>{result.order_number}</strong> is paid. Thank you.
        </p>
        <Link
          href={`/order-confirmation/${result.order_id}`}
          className="mt-6 inline-block rounded-md bg-amber-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-amber-800"
        >
          View order
        </Link>
      </div>
