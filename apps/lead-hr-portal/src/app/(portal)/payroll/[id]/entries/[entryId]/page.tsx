...
export default function EntryEditPage({
  ...
  const [adjustmentHydrated, setAdjustmentHydrated] = useState(false);
  ...
  useEffect(() => {
    ...
    if (entry && !adjustmentHydrated) {
      setAdjustmentAmount(entry.adjustment_amount ? String(entry.adjustment_amount) : "");
      setAdjustmentNote(entry.adjustment_note ?? "");
      setAdjustmentHydrated(true);
    }
  }, [entry, hydrated, adjustmentHydrated]);
  ...
  async function downloadPayslip() {
    ...
    try {
      const res = await apiClient.get(`/api/payroll/entries/${entryId}/payslip`, {
        responseType: "blob",
      });
      ...
      setPayslipError((err as Error).message);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="rounded-lg border border-stone-200 bg-white p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            >
              ...
              {downloadingPayslip ? "Preparing…" : "Download pay slip"}
            </button>
            {isLocked && (
        ...
        {payslipError && (
          <p className="mt-2 text-xs text-red-600">{payslipError}</p>
        )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        {/* Manual adjustment — bonus (positive) or withholding (negative) */}
        <div className="lg:col-span-2 rounded-lg border border-stone-200 bg-white p-6 space-y-4">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-black border-b border-stone-100 pb-2">
            <PlusCircle className="h-4 w-4" />
            Owed pay / bonus / withholding
          </h2>
          <p className="text-xs text-stone-500">
            A one-off adjustment on top of everything else — positive to pay a bonus or
            owed back pay, negative to withhold. Always requires a comment.
          </p>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              label="Amount (₦)"
              hint="Positive to add, negative to withhold — e.g. -2000"
            >
              <input
                type="number"
                step="0.01"
          ...
          <Field label="Comment" hint="Required — explain what this is for">
            <textarea
              value={adjustmentNote}
              onChange={(e) => setAdjustmentNote(e.target.value)}
              disabled={isLocked}
              rows={2}
              placeholder="e.g. Performance bonus for August, or withheld for unreturned uniform"
              className={inputCls}
            />
          </Field>

          {adjustmentMutation.isError && (
            <ErrorState message={(adjustmentMutation.error as Error).message} />
          )}
